                                        ...
                                        int up)
{
    ...
    if ((old[i] > 3) &&
        (memscan(new_ + 2, old[i], USB_KBD_BOOT_REPORT_SIZE - 2) ==
         new_ + USB_KBD_BOOT_REPORT_SIZE))
    {
...
static int usb_kbd_irq_worker(struct usb_device *dev)
{
    ...
    if ((data->new_[2] > 3) && (data->old[2] == data->new_[2]))
        res |= usb_kbd_translate(data, data->new_[2], data->new_[0], 2);

    if (res == 1)
        usb_kbd_setled(dev);

    memcpy(data->old, data->new_, USB_KBD_BOOT_REPORT_SIZE);
