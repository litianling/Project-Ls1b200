    ...
    switch (ed->type)
    {
        case PIPE_CONTROL:
            ...
            if (ohci->ed_controltail == NULL)
            	...
            	ohci_writel(K0_TO_PHYS((unsigned int)ed), &ohci->regs->ed_controlhead);	// physical
            else
                //ohci->ed_controltail->hwNextED = (unsigned long)ed;
            	ohci->ed_controltail->hwNextED = K0_TO_PHYS((unsigned int)ed); 			// physical
        ...
        case PIPE_BULK:
            ...
            if (ohci->ed_bulktail == NULL)
            	...
            	ohci_writel(K0_TO_PHYS((unsigned int)ed), &ohci->regs->ed_bulkhead);	// physical
            else
                //ohci->ed_bulktail->hwNextED = (unsigned long)ed;
            	ohci->ed_bulktail->hwNextED = K0_TO_PHYS((unsigned int)ed);				// physical
        ...
        case PIPE_INTERRUPT:
            ...
            for (i = 0; i < ep_rev(6, interval); i += inter)
            {
                ...
                *ed_p = K0_TO_PHYS((unsigned int)ed);					// physical
                          ...
                          int interval)
{
    ...
    clean_dcache((unsigned int)buffer, data_len);
    ...
    switch (usb_pipetype(pipe))
    {
        ...
        case PIPE_CONTROL:
            ...
            clean_dcache((unsigned int)setup, 8);
                              ...
                              struct devrequest *cmd)
{
    ...
    ALLOC_ALIGN_BUFFER(unsigned char, databuf, 16, sizeof(unsigned int));
                             ...
                             int interval)
{
    ...
    if (usb_pipein(pipe) && dev->status == 0 && dev->act_len)
        ...
        clean_dcache_nowrite((unsigned int)buffer, dev->act_len);
...
static int hc_start(ohci_t *ohci)
{
    ...
    //ohci_writel((uintptr_t)ohci->hcca, &ohci->regs->hcca); /* reset clears this */
    ohci_writel(K0_TO_PHYS((unsigned int)ohci->hcca), &ohci->regs->hcca);
    ...
    ohci_writel((roothub_a(ohci) | RH_A_NPS) & ~RH_A_PSM, &ohci->regs->roothub.a);
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
    ...
    if ((unsigned int)&g_HCCA[0] & 0xff)            /* align the storage */
    {
    ...
    INFO("aligned g_hcca %p", ohci->hcca);
    ...
    ohci->regs = (struct ohci_regs *)CONFIG_SYS_USB_OHCI_REGS_BASE;

    ohci->flags = 0;
    ohci->slot_name = CONFIG_SYS_USB_OHCI_SLOT_NAME;

    if (hc_reset(ohci) < 0)
    {
        hc_release_ohci(ohci);
        ERR("can't reset usb-%s", ohci->slot_name);
#ifdef CONFIG_SYS_USB_OHCI_BOARD_INIT
...
int submit_control_msg(struct usb_device *dev,
                       ...
                       struct devrequest *setup)
{
    return _ohci_submit_control_msg(&g_OHCI, dev, pipe, buffer, transfer_len, setup);
}
#endif
