				                     ...
				                     int part_num, unsigned int disksig)
{
	...
	if (blk_dread(dev_desc, ext_part_sector, 1, (unsigned long *)buffer) != 1)
	{
                                  ...
                                  unsigned disksig)
{
	...
	for (i = 0; i < 4; i++, pt++)
	{
		    ...
		    (ext_part_sector == 0 || is_extended(pt->sys_ind) == 0))
		{
			...
			strcpy((char *)info->type, "loongson");
...
int part_get_info_dos(struct blk_desc *dev_desc, int part,
		              struct disk_partition *info)
{
    int ret = part_get_info_extended(dev_desc, 0, 0, 1, part, info, 0);
    if (ret == 0)
    ...
    if (part_get_info_dos(desc, 1, &mypart) == 0)
    {
        if (fat_set_blk_dev(desc, &mypart) == 0)
        {
            if (fat_exists(fn))
            {
                ...
                char buf[512];
                printf("found file: %s\r\n", fn);
                count = file_fat_read(fn, buf, 512);
                printf("%s\r\n", buf);
            }
            else
            {
