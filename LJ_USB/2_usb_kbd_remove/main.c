...
int main(void)
{
    ...
    LS1B_MUX_CTRL1 &= ~MUX_CTRL1_USB_SHUT;  // 将LS1B_MUX_CTRL1寄存器第十一位归零
    delay_ms(1);
    LS1B_MUX_CTRL1 |= MUX_CTRL1_USB_RESET;  // 将LS1B_MUX_CTRL1寄存器第三十一位置一
    delay_ms(1);
    ...
    drv_usb_kbd_init();                     // 键盘的驱动初始化
    while (1)
    {
        if (usb_kbd_testc())                // 检测是否有输入
        {
            int ch = usb_kbd_getc();        // 读取键盘输入，返回输入字符
            //printk("%c\n", ch);             // 打印输入字符
        }
        delay_ms(5);
    }
