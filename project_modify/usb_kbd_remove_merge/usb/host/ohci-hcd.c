                     ...
                     struct devrequest *setup)
{
    ...
    /* 对于 URB 的私有部分，我们需要 TD 的数量（大小） */
    switch (usb_pipetype(pipe))                                     // 获取管道类型【*】
    {
        case PIPE_BULK:         /* 批量管道每 4096 字节一个 TD */
    ...
    for (i = 0; i < size; i++)
    {
        purb_priv->td[i] = td_alloc(ohci_dev, dev);     // 向OHCI下分配设备【*】
        if (!purb_priv->td[i])                          // 如果设备不存在
        {
            purb_priv->length = i;                      // 长度
            urb_free_priv(purb_priv);                   // 释放与此 URB 关联的 HCD 私有数据【*】
            ERR("sohci_submit_job: ENOMEM");
            return -1;
        }
    }

    if (ed->state == ED_NEW || (ed->state & ED_DEL))    // 判断端点状态
    {
        urb_free_priv(purb_priv);                       // 释放与此 URB 关联的 HCD 私有数据【*】
    ...
    /* 如果还没有，则将 ed 链接到链中 */
    if (ed->state != ED_OPER)
        ep_link(ohci, ed);                              // 通过写寄存器将 ed 链接到 HC 链之一【*】
                       ...
                       unsigned long pipe, int interval, int load)
{
    ...
    ed = ed_ret = &ohci_dev->ed[(usb_pipeendpoint(pipe) << 1) |
                                (usb_pipecontrol(pipe) ? 0 : usb_pipeout(pipe))];       // 输入管道/控制管道/输出管道【*】
                    ...
                    struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
    ...
    td->index = index;      // 指针
    td->data = (unsigned int/*uintptr_t*/)data; // 接收数据
                          ...
                          int interval)
{
    ...
    urb->td_cnt = 0;
    if (data_len)                                                       // 数据长度不为0，将数据存储在bufer之中
        data = buffer;
    else
        data = 0;

    switch (usb_pipetype(pipe))                                         // 根据管道类型配置【*】
    {
        case PIPE_BULK:
            info = usb_pipeout(pipe) ? TD_CC | TD_DP_OUT : TD_CC | TD_DP_IN ;
            while (data_len > 4096)
            {
                td_fill(ohci, info | (cnt ? TD_T_TOGGLE:toggle), data, 4096, dev, cnt, urb);  // 为这个 URB 排队下一个 TD【*】
        ...
        case PIPE_CONTROL:
            ...
            {
                /* 启动控制列表t */
                ohci_writel(OHCI_CLF, &ohci->regs->cmdstatus);                              // 写32位寄存器开始批量传输【*】
            }
            break;
...
static ohci_dev_t *ohci_get_ohci_dev(ohci_t *ohci, int devnum, int intr)
{
    ...
    /* 首先看看我们是否已经有这个开发的 ohci_dev。 */
    for (i = 0; i < NUM_INT_DEVS; i++)
    {
        if (ohci->int_dev[i].devnum == devnum)          // 根据设备号在int_dev查找OHCI设备，返回
    ...
    for (i = 0; i < NUM_INT_DEVS; i++)
    {
        if (ohci->int_dev[i].devnum == -1)              // 没找到的话，找个空余位置新建设备并返回
        {
                             ...
                             int interval)
{
    ...
    if (sohci_submit_job(ohci, ohci_dev, urb, setup) < 0)                // 获得转移请求【*】（传输在它里面！！！！！）
    {
...
static int hc_start(ohci_t *ohci)
{
    ...
    //ohci_writel((uintptr_t)ohci->hcca, &ohci->regs->hcca); /* 复位清除 */
...
//    invalidate_dcache_hcca(ohci->hcca);
    ...
    if ((ohci->hcca->done_head != 0) && !(ohci->hcca->done_head & 0x01))    // OCHI与HCCA正确
    {
        ints =  OHCI_INTR_WDH;
    }
    else                                                                    // 错误
    ...
    if (ints & OHCI_INTR_UE)                                                        // 中断发生不可恢复的错误
    {
        ...
        hc_reset(ohci);                                                             // 释放OCHI设备【*】复位HC和总线
        return -1;
    }
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
    ...
    memset((void *)ohci, 0, sizeof(ohci_t));        // 这个OHCI初始化为0

    if ((unsigned int)&g_HCCA[0] & 0xff)            // 对齐存储：&g_HCCA[0]地址最后两位如果不是0x00
    {                                               //          它和0xff与运算之后必不为0x00，发生错误
    ...
    ohci->regs = (struct ohci_regs *)CONFIG_SYS_USB_OHCI_REGS_BASE;     // 初始化OHCI第一个寄存器基地址0xBFE08000
