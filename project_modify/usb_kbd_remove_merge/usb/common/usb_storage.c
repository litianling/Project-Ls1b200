...
#endif
                if (us->pusb_dev->status & USB_ST_STALLED)          // 如果ST被挂起
                {
...
static int usb_stor_CB_comdat(struct scsi_cmd *srb, struct us_data *us)
{
    ...
    while (retry--)
    {
        ...
        if (result < 0)
        {
            if (us->pusb_dev->status & USB_ST_STALLED)                      // 如果TD被挂起了
            {
...
do_retry:
    ...
    /* 发出命令 */
    result = usb_stor_CB_comdat(psrb, us);                                              // 发出CB存储器命令【*】
    DEBUG("auto request returned %d\n", result);

    /* 如果这是一个 CBI 协议，获取 IRQ */
    if (us->protocol == US_PR_CBI)
        status = usb_stor_CBI_get_status(psrb, us);                                     // 获取存储器状态【*】
...
retry_it:
        ...
        srb->datalen = block_dev->blksz * smallblks;        // 数据长度
        srb->pdata = (unsigned char *)buf_addr;             // 数据区地址
        if (usb_write_10(srb, ss, start, smallblks))        // 开始读大容量存储器【*】配置完一系列命令后调用transport发出去
        {
            DEBUG("Write ERROR\n");
            ss->flags &= ~USB_READY;                        // 标志：USB未就绪
            usb_request_sense(srb, ss);                     // 同上【*】
                      ...
                      struct us_data *ss)
{
    ...
    switch (ss->protocol)
    {
        case US_PR_CB:                                              // 协议支持的传输模式有：Control/Bulk w/o interrupt
            DEBUG("Control/Bulk\n");
            ss->transport = usb_stor_CB_transport;                  // 控制/批量（CB）传输程序【*】
                      ...
                      struct blk_desc *dev_desc)
{
    ...
    dev_desc->target = dev->devnum;                                 // 目标为设备号
