...
int usb_alloc_new_device(struct udevice *controller,struct usb_device **devp)
{
    ...
    usb_dev[dev_index].parent = NULL;               // 此设备接在总线上，父集线器为0
                              ...
                              struct usb_device *parent)
{
    ...
    if (err)
        return err;
    err = usb_hub_port_reset(dev, parent);      // USB集线器接口复位【*】（参数：当前设备与父设备）
    if (err)
        return err;
 ...
 */
    ...
    usb_set_maxpacket(dev);                                     // 设置配置描述符最大值【*】
...
int usb_new_device(struct usb_device *dev)
{
    ...
    // 现在探测设备是否是集线器【*】【是集线器直接返回0，不是集线器先配置集线器后返回（配置不出错也是返回0）】
