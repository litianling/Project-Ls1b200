				                     ...
				                     int part_num, unsigned int disksig)
{
	...
	i=test_block_type(buffer);                                                     // 检查缓存区类型【*】
	if (i != DOS_MBR)
	{
                                  ...
                                  unsigned disksig)
{
	...
	/* 打印所有主/逻辑分区 */
	pt = (dos_partition_t *) (buffer + DOS_PART_TBL_OFFSET);                           // 强制数据类型转换
	for (i = 0; i < 4; i++, pt++)
	{
		/*
		 * fdisk 不显示不在 MBR 中的扩展分区
		...
		if (((pt->boot_ind & ~0x80) == 0) &&
		    (pt->sys_ind != 0) &&
		    (part_num == which_part) &&
		    (ext_part_sector == 0 || is_extended(pt->sys_ind) == 0))                   // 是不是扩展的【*】
		{
		...
		/* 反向 engr fdisk part# 分配规则! */
		if ((ext_part_sector == 0) ||
		    (pt->sys_ind != 0 && !is_extended (pt->sys_ind)))                         // 同上【*】
		{
			part_num++;
		}
	}
