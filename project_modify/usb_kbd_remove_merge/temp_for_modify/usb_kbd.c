 ...
 */
void *memscan(void *addr, int c, size_t size)   //传入临时现态地址+2，临时次态的第i个值，报表值-2
{
    int i;
    unsigned char *ptr = (unsigned char *)addr;
    
    for (i=0; i<size; i++)   //遍历临时现态的地址，若发现有和临时次态第i值相等的值 返回其地址+i
    {
        if ((int)ptr[i] == c)
        {
            return (void *)(ptr + i);
...
static unsigned int usb_kbd_service_key(struct usb_device *dev,
                                        int i,
                                        int up) //传入(dev，报表的低价几个字节号，按下状态)
{
    unsigned int res = 0;
    struct usb_kbd_pdata *data = dev->privptr;
    unsigned char *new_;   //临时的存储空间  //临时现态
    unsigned char *old;     //临时次态

    if (up)
    {
        new_ = data->old;    //临时现态=次态
        old  = data->new_;   //临时次态=现态
    }
    else //按下就是正常的赋值
    {
        new_ = data->new_;    //临时现态=现态
        old  = data->old;     //临时次态=次态
    }

    if ((old[i] > 3) && memscan(new_ + 2, old[i], USB_KBD_BOOT_REPORT_SIZE - 2) == NULL) //对临时次态与临时现态进行判定
    {   //注意old[i]中的i必>=2
        res |= usb_kbd_translate(data, old[i], data->new_[0], up);
    }
...
{
    struct usb_kbd_pdata *data = dev->privptr;
    int i, res = 0;          //res为设置led的标志位

    /* No combo key pressed */
    if (data->new_[0] == 0x00)
        data->flags &= ~USB_KBD_CTRL;     //标志位赋值无ctrl键按下   CTRL标志=(1<<3)
    /* Left or Right Ctrl pressed */
    else if ((data->new_[0] == LEFT_CNTR) || (data->new_[0] == RIGHT_CNTR))
        data->flags |= USB_KBD_CTRL;      //标志位赋值  CTRL按下标志

    for (i = 2; i < USB_KBD_BOOT_REPORT_SIZE; i++)  //从第二个字节开始是普通按键的报表
    {
        res |= usb_kbd_service_key(dev, i, 0);        //up=0的作用清除按住不放的重复标志
        res |= usb_kbd_service_key(dev, i, 1);        //up=1若按下或弹起正常译码（就是次态！=现态时正常译码）
    }

    /* Key is still pressed */
    if ((data->new_[2] > 3) && (data->old[2] == data->new_[2]))    //普通按键的报表值从3起始  次态=现态 表示按键一直按下 old为次态 new为现态
        res |= usb_kbd_translate(data, data->new_[2], data->new_[0], 2);  //次态和现态的值相等就一直自加

    if (res == 1)
        usb_kbd_setled(dev);

    memcpy(data->old, data->new_, USB_KBD_BOOT_REPORT_SIZE);  //所有操作执行过后才将现态赋值给次态
...
//    usb_kbd_dev = (struct usb_device *)dev->priv;
    ...
    usb_kbd_poll_for_event(m_usb_kbd_dev);
...
//  data->new_ = memalign(USB_DMA_MINALIGN, roundup(USB_KBD_BOOT_REPORT_SIZE, USB_DMA_MINALIGN));
    data->new_ = aligned_malloc(USB_DMA_MINALIGN, roundup(USB_KBD_BOOT_REPORT_SIZE, USB_DMA_MINALIGN));
    ...
    data->intpktsize = min(usb_maxpacket(dev, data->intpipe), USB_KBD_BOOT_REPORT_SIZE);
                    ...
                    false) < 0)
    {
        ...
        return 0;   /* Abort, we don't want to use that non-functional keyboard. */
    }
