...
typedef struct
{
	...
	int         last_cluster;  /* 一旦我们读取了最后一个集群就设置 */
	int         is_root;       /* 是根目录的迭代器 */
...
static int fat_itr_resolve(fat_itr_t *itr, const char *path, unsigned type)
{
	...
	/* 移除任何额外的前导斜杠: */
	while (path[0] && ISDIRDELIM(path[0]))     // 移除文件路径中额外的前导斜杠
		path++;
	/* 遍历到末尾了吗? */
	if (strlen(path) == 0)                     // 计算字符串长度【*】计算移除斜杠后的名字是否为空
	{
		if (!(type & TYPE_DIR))
			return -ENOENT;
		return 0;
	...
	/* 查找下一个路径条目的长度: */
	next = path;
	while (next[0] && !ISDIRDELIM(next[0]))    // 移除next的前导斜杠
		next++;
	...
	while (fat_itr_next(itr))                              // 跳转到目录中的下一个条目【*】
	{
		int match = 0;
		unsigned n = max(strlen(itr->name), (size_t)(next - path));   // 最大值【*】

		/* 检查长名称和短名称: */
		if (!strncasecmp(path, itr->name, n))             // 检查name【*】
			match = 1;
		else if (itr->name != itr->s_name &&
			 !strncasecmp(path, itr->s_name, n))          // 同上【*】
			match = 1;
		...
		{
			/*
			 * 如果 next 不为空，那么我们的情况如下：/path/to/realfile/nonsense
			 */
...
int fat_exists(const char *filename)
{
	...
	itr = malloc_cache_aligned(sizeof(fat_itr_t));         // 迭代器iterator（itr）空间分配【*】包含集群、数据、根目录迭代器等
	if (!itr)
		return 0;
		             ...
		             loff_t maxsize, loff_t *actread)
{
	...
	ret = fat_itr_resolve(itr, filename, TYPE_FILE);                           // 遍历目录结构来解析请求的路径【*】
	if (ret)
		goto out_free_both;
