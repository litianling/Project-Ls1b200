 ...
 * Copyright (c) 2008, Excito Elektronik i Sk氓ne AB
...
#define EHCI_MEM_POOL   0

#if (EHCI_MEM_POOL)
static void ehci_qtd_pool_init(void);
#endif
		          ...
		          struct ehci_hccr **hccr, struct ehci_hcor **hcor)
{
    ...
    *hcor = (struct ehci_hcor *)0xBFE00010;
    
    LS1B_MUX_CTRL1 &= ~MUX_CTRL1_USB_SHUT;
    DELAY_MS(1);
    
    LS1B_MUX_CTRL1 &= ~MUX_CTRL1_USB_RESET;
    DELAY_MS(10);
    LS1B_MUX_CTRL1 |= MUX_CTRL1_USB_RESET;

#elif defined(LS1C)
    *hccr = (struct ehci_hccr *)0xBFE20000;
    *hcor = (struct ehci_hcor *)0xBFE20010;
...
#if (CONFIG_EHCI_IS_TDI)
#if defined(LS1B) || defined(LS1C)
#error "Loongson SoC is no TDI!"
#endif
...
static int ehci_get_port_speed(struct ehci_ctrl *ctrl, unsigned int reg)
{
...
static void ehci_set_usbmode(struct ehci_ctrl *ctrl)
{
    ...
    reg_ptr = (unsigned int *)((unsigned char *)&ctrl->hcor->or_usbcmd + USBMODE);
...
static void ehci_powerup_fixup(struct ehci_ctrl *ctrl, unsigned int *status_reg,
                               unsigned int *reg)
{
    DELAY_MS(50);
}

static unsigned int *ehci_get_portsc_register(struct ehci_ctrl *ctrl, int port)
{
    ...
    return (unsigned int *)&ctrl->hcor->or_portsc[port];
}

static int handshake(unsigned int *ptr, unsigned int mask, unsigned int done, int usec)
{
    unsigned int result;
    do
    {
...
static int ehci_reset(struct ehci_ctrl *ctrl)
{
    ...
    ret = handshake((unsigned int *)&ctrl->hcor->or_usbcmd, CMD_RESET, 0, 250 * 1000);
...
static int ehci_shutdown(struct ehci_ctrl *ctrl)
{
    ...
    cmd &= ~(CMD_PSE | CMD_ASE);
    ehci_writel(&ctrl->hcor->or_usbcmd, cmd);
    ret = handshake((unsigned int *)&ctrl->hcor->or_usbsts, STS_ASS | STS_PSS, 0, 100 * 1000);
    ...
    if (!ret)
    {
        ...
        cmd &= ~CMD_RUN;
        ehci_writel(&ctrl->hcor->or_usbcmd, cmd);
        ret = handshake((unsigned int *)&ctrl->hcor->or_usbsts, STS_HALT, STS_HALT, HCHALT_TIMEOUT);
    }
...
static int ehci_td_buffer(struct qTD *td, void *buf, size_t sz)
{
    ...
    clean_dcache(addr, sz);

    idx = 0;
    while (idx < QT_BUFFER_CNT)
    {
        //td->qt_buffer[idx] = virt_to_phys((void *)addr);
        td->qt_buffer[idx] = K0_TO_PHYS(addr);                  // physical
...
static void ehci_update_endpt2_dev_n_port(struct usb_device *udev, struct QH *qh)
{
    unsigned char portnr = 0;
    unsigned char hubaddr = 0;
    ...
    qh->qh_endpt2 |= QH_ENDPT2_PORTNUM(portnr) | QH_ENDPT2_HUBADDR(hubaddr);
}
...
static int ehci_enable_async(struct ehci_ctrl *ctrl)
{
    ...
    //cmd |= CMD_ASE;
    cmd |= CMD_ASE | CMD_RUN;           /* PMON do like this */
    ehci_writel(&ctrl->hcor->or_usbcmd, cmd);

    ret = handshake((unsigned int *)&ctrl->hcor->or_usbsts, STS_ASS, STS_ASS, 100 * 1000);
    if (ret < 0)
...
static int ehci_disable_async(struct ehci_ctrl *ctrl)
{
    ...
    ret = handshake((unsigned int *)&ctrl->hcor->or_usbsts, STS_ASS, 0, 100 * 1000);
    if (ret < 0)
 ...
 * 用 pool 管理 qTD
 */
#define QTD_POOL_MAX    32

static struct qTD qtd_pool[QTD_POOL_MAX];

static void ehci_qtd_pool_init(void)
{
    int i;
    for (i=0; i<QTD_POOL_MAX; i++)
        qtd_pool[i].unused[0] = 1;
}

static struct qTD *ehci_qtd_pool_alloc(int count)
{
    if (count == 0)
        return NULL;
    ...
    if (count == 1)
    {
        ...
        for (i=0; i<QTD_POOL_MAX; i++)
        {
            if (qtd_pool[i].unused[0])
            {
                qtd_pool[i].unused[0] = 0;
                return &qtd_pool[i];
            }
        }
...
static void ehci_qtd_pool_free(struct qTD *qtd, int count)
{
    if (count == 0)
    {
        qtd->unused[0] = 1;
...
static int ehci_submit_async(struct usb_device *dev, unsigned long pipe,
                             void *buffer, int length, struct devrequest *req)
{
    ...
    unsigned long ts;
    unsigned int *tdp;
    unsigned int endpt, maxpacket, token, usbsts, qhtoken;
    unsigned int c, toggle;
    int timeout;
    int ret = 0;
...
#warning CONFIG_SYS_MALLOC_LEN may be too small for EHCI
...
#if (EHCI_MEM_POOL)
    qtd = ehci_qtd_pool_alloc(qtd_count);
#else
    // qtd = memalign(USB_DMA_MINALIGN, qtd_count * sizeof(struct qTD));
    qtd = aligned_malloc(qtd_count * sizeof(struct qTD), USB_DMA_MINALIGN);
#endif
    if (qtd == NULL)
    {
     ...
     */
    //qh->qh_link = virt_to_phys(&ctrl->qh_list) | QH_LINK_TYPE_QH;
    qh->qh_link = K0_TO_PHYS((unsigned int)&ctrl->qh_list) | QH_LINK_TYPE_QH;   // physical
    c = (dev->speed != USB_SPEED_HIGH) && !usb_pipeendpoint(pipe);
    maxpacket = usb_maxpacket(dev, pipe);
    endpt = QH_ENDPT1_RL(8) | QH_ENDPT1_C(c) |
            QH_ENDPT1_MAXPKTLEN(maxpacket) | QH_ENDPT1_H(0) |
            QH_ENDPT1_DTC(QH_ENDPT1_DTC_DT_FROM_QTD) |
            QH_ENDPT1_ENDPT(usb_pipeendpoint(pipe)) |
            QH_ENDPT1_I(0) | QH_ENDPT1_DEVADDR(usb_pipedevice(pipe));
    ...
    if (req != NULL)
    {
        ...
        /* Update previous qTD! */
        //*tdp = virt_to_phys(&qtd[qtd_counter]);
        *tdp = K0_TO_PHYS((unsigned int)&qtd[qtd_counter]);     // physical
    ...
    if (length > 0 || req == NULL)
    {
        ...
        do
        {
            ...
            qtd[qtd_counter].qt_next = QT_NEXT_TERMINATE;
            qtd[qtd_counter].qt_altnext = QT_NEXT_TERMINATE;
            token = QT_TOKEN_DT(toggle) | QT_TOKEN_TOTALBYTES(xfr_bytes) |
                    QT_TOKEN_IOC(req == NULL) | QT_TOKEN_CPAGE(0) | QT_TOKEN_CERR(3) |
                    QT_TOKEN_PID(usb_pipein(pipe) ? QT_TOKEN_PID_IN : QT_TOKEN_PID_OUT) |
                    QT_TOKEN_STATUS(QT_TOKEN_STATUS_ACTIVE);
            qtd[qtd_counter].qt_token = token;

            if (ehci_td_buffer(&qtd[qtd_counter], buf_ptr, xfr_bytes))
            {
    ...
    if (req != NULL)
    {
        ...
        token = QT_TOKEN_DT(1) | QT_TOKEN_TOTALBYTES(0) |
                ...
                QT_TOKEN_PID(usb_pipein(pipe) ? QT_TOKEN_PID_OUT : QT_TOKEN_PID_IN) |
                QT_TOKEN_STATUS(QT_TOKEN_STATUS_ACTIVE);
        qtd[qtd_counter].qt_token = token;
        /* Update previous qTD! */
        //*tdp = virt_to_phys(&qtd[qtd_counter]);
    ...
    //ctrl->qh_list.qh_link = virt_to_phys(qh) | QH_LINK_TYPE_QH;
    ctrl->qh_list.qh_link = K0_TO_PHYS((unsigned int)qh) | QH_LINK_TYPE_QH; // physical

    /* Flush dcache */
    //flush_dcache_range((unsigned long)&ctrl->qh_list, ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
    clean_dcache((unsigned int)&ctrl->qh_list, sizeof(struct QH));
    
    //flush_dcache_range((unsigned long)qh, ALIGN_END_ADDR(struct QH, qh, 1));
    clean_dcache((unsigned int)qh, sizeof(struct QH));
    
    //flush_dcache_range((unsigned long)qtd, ALIGN_END_ADDR(struct qTD, qtd, qtd_count));
    clean_dcache((unsigned int)qtd, sizeof(struct qTD) * qtd_count);
    ...
    do
    {
         ...
         */
        /* Invalidate dcache */
        //invalidate_dcache_range((unsigned long)&ctrl->qh_list, ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
        clean_dcache_nowrite((unsigned int)&ctrl->qh_list, sizeof(struct QH));
        
        //invalidate_dcache_range((unsigned long)qh, ALIGN_END_ADDR(struct QH, qh, 1));
        clean_dcache_nowrite((unsigned int)qh, sizeof(struct QH));
        
        //invalidate_dcache_range((unsigned long)qtd, ALIGN_END_ADDR(struct qTD, qtd, qtd_count));
        clean_dcache_nowrite((unsigned int)qtd, sizeof(struct qTD) * qtd_count);
    ...
    qhtoken = qh->qh_overlay.qt_token;

    //ctrl->qh_list.qh_link = virt_to_phys(&ctrl->qh_list) | QH_LINK_TYPE_QH;
    ctrl->qh_list.qh_link = K0_TO_PHYS((unsigned int)&ctrl->qh_list) | QH_LINK_TYPE_QH;
    //flush_dcache_range((unsigned long)&ctrl->qh_list, ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
    clean_dcache((unsigned int)&ctrl->qh_list, sizeof(struct QH));
    ...
    if (buffer != NULL && length > 0)
        //invalidate_dcache_range((unsigned long)buffer, ALIGN((unsigned long)buffer + length, ARCH_DMA_MINALIGN));
        clean_dcache_nowrite((unsigned int)buffer, length);
    ...
    {
        DEBUG("TOKEN=%#x\n", qhtoken);
        switch (QT_TOKEN_GET_STATUS(qhtoken) & ~(QT_TOKEN_STATUS_SPLITXSTATE | QT_TOKEN_STATUS_PERR))
        {
            case 0:
    ...
    else
    {
        ...
        DEBUG("dev=%u, usbsts=%#x, p[1]=%#x, p[2]=%#x\n",
               dev->devnum,
               (unsigned int)ehci_readl(&ctrl->hcor->or_usbsts),
               (unsigned int)ehci_readl(&ctrl->hcor->or_portsc[0]),
               (unsigned int)ehci_readl(&ctrl->hcor->or_portsc[1]));
#endif
    }

#if (EHCI_MEM_POOL)
    ehci_qtd_pool_free(qtd, qtd_count);
#else
    aligned_free(qtd);
#endif
                            ...
                            void *buffer, int length, struct devrequest *req)
{
            ...
            switch (req->value)
            {
                ...
                case USB_PORT_FEAT_RESET:
                    if ((reg & (EHCI_PS_PE | EHCI_PS_CS)) == EHCI_PS_CS &&
                        !ehci_is_TDI() && EHCI_PS_IS_LOWSPEED(reg))
                    {
                    ...
                    else
                    {
                        ...
                        ehci_writel(status_reg, reg);
                        /*
                         * caller must wait, then call GetPortStatus usb 2.0
                         * specification say 50 ms resets on root
                         */
                        ctrl->ops.powerup_fixup(ctrl, status_reg, &reg);

                        ehci_writel(status_reg, reg & ~EHCI_PS_PR);
                        /*
                         * A host controller must terminate the reset and stabilize
                         * the state of the port within 2 milliseconds
                         */
                        ret = handshake(status_reg, EHCI_PS_PR, 0, 2 * 1000);
                        if (!ret)
                        {
                            reg = ehci_readl(status_reg);
                            if ((reg & (EHCI_PS_PE | EHCI_PS_CS)) == EHCI_PS_CS && !ehci_is_TDI())
                            {
    ...
    DELAY_MS(1);
    len = min3(srclen, (int)req->length, length);
...
static void ehci_setup_ops(struct ehci_ctrl *ctrl, const struct ehci_ops *ops)
{
    ...
    else
    {
        ...
        if (!ctrl->ops.get_portsc_register)
            ctrl->ops.get_portsc_register = ehci_get_portsc_register;
    }
}
...
static int ehci_common_init(struct ehci_ctrl *ctrl, unsigned int tweaks)
{
    ...
    //qh_list->qh_link   = virt_to_phys(qh_list) | QH_LINK_TYPE_QH;
    qh_list->qh_link   = K0_TO_PHYS((unsigned int)qh_list) | QH_LINK_TYPE_QH;
    qh_list->qh_endpt1 = QH_ENDPT1_H(1) | QH_ENDPT1_EPS(USB_SPEED_HIGH);
    qh_list->qh_overlay.qt_next    = QT_NEXT_TERMINATE;
    qh_list->qh_overlay.qt_altnext = QT_NEXT_TERMINATE;
    qh_list->qh_overlay.qt_token   = QT_TOKEN_STATUS(QT_TOKEN_STATUS_HALTED);

    //flush_dcache_range((unsigned long)qh_list, ALIGN_END_ADDR(struct QH, qh_list, 1));
    clean_dcache((unsigned int)qh_list, sizeof(struct QH));
    
    /* Set async. queue head pointer. */
    //ehci_writel(&ctrl->hcor->or_asynclistaddr, virt_to_phys(qh_list));
    ehci_writel(&ctrl->hcor->or_asynclistaddr, K0_TO_PHYS((unsigned int)qh_list));
    ...
    periodic->qh_link = QH_LINK_TERMINATE;
    periodic->qh_overlay.qt_next = QT_NEXT_TERMINATE;
    periodic->qh_overlay.qt_altnext = QT_NEXT_TERMINATE;

    //flush_dcache_range((unsigned long)periodic, ALIGN_END_ADDR(struct QH, periodic, 1));
    clean_dcache((unsigned int)periodic, sizeof(struct QH));
    ...
    if (ctrl->periodic_list == NULL)
        ...
        ctrl->periodic_list = aligned_malloc(1024 * 4, 4096);
    ...
    for (i = 0; i < 1024; i++)
    {
        ctrl->periodic_list[i] = (unsigned long)periodic | QH_LINK_TYPE_QH;
    }

    //flush_dcache_range((unsigned long)ctrl->periodic_list,
    //                    ALIGN_END_ADDR(unsigned int, ctrl->periodic_list, 1024));
    clean_dcache((unsigned int)ctrl->periodic_list, sizeof(unsigned int) * 1024);
    
    /* Set periodic list base address */
    ehci_writel(&ctrl->hcor->or_periodiclistbase, (unsigned long)ctrl->periodic_list);
    ...
    //    put_unaligned(get_unaligned(&descriptor.hub.wHubCharacteristics) | 0x80,
    //                                &descriptor.hub.wHubCharacteristics);
        descriptor.hub.wHubCharacteristics |= 0x80;

    /* Port Power Control */
    if (HCS_PPC(reg))
    //    put_unaligned(get_unaligned(&descriptor.hub.wHubCharacteristics) | 0x01,
    //                                &descriptor.hub.wHubCharacteristics);
...
#define NEXT_QH(qh) (struct QH *)((unsigned long)((qh)->qh_link) & ~0x1f)

static int enable_periodic(struct ehci_ctrl *ctrl)
{
    unsigned int cmd;
    ...
    ret = handshake((unsigned int *)&hcor->or_usbsts, STS_PSS, STS_PSS, 100 * 1000);
    if (ret < 0)
    {
...
static int disable_periodic(struct ehci_ctrl *ctrl)
{
    unsigned int cmd;
    struct ehci_hcor *hcor = ctrl->hcor;
    int ret;
    ...
    ret = handshake((unsigned int *)&hcor->or_usbsts, STS_PSS, 0, 100 * 1000);
    if (ret < 0)
    {
                                                ...
                                                int interval)
{
    ...
    result->pipe = pipe;
    // result->first = memalign(USB_DMA_MINALIGN, sizeof(struct QH) * queuesize);
    result->first = aligned_malloc(sizeof(struct QH) * queuesize, USB_DMA_MINALIGN);
    if (!result->first)
    {
        DEBUG("ehci intr queue: out of memory\n");
        goto fail2;
    ...
    // result->tds = memalign(USB_DMA_MINALIGN, sizeof(struct qTD) * queuesize);
    result->tds = aligned_malloc(sizeof(struct qTD) * queuesize, USB_DMA_MINALIGN);
    ...
    for (i = 0; i < queuesize; i++)
    {
        ...
        qh->qh_link = (unsigned long)(qh+1) | QH_LINK_TYPE_QH;
        if (i == queuesize - 1)
            qh->qh_link = QH_LINK_TERMINATE;

        qh->qh_overlay.qt_next = (unsigned long)td;
        qh->qh_overlay.qt_altnext = QT_NEXT_TERMINATE;
        ...
        td->qt_token = QT_TOKEN_DT(toggle) |
                       (elementsize << 16) |
                       ((usb_pipein(pipe) ? 1 : 0) << 8) |  /* IN/OUT token */
                       0x80;                                /* active */

        td->qt_buffer[0] = (unsigned long)buffer + i * elementsize;
    ...
    //flush_dcache_range((unsigned long)buffer, ALIGN_END_ADDR(char, buffer, queuesize * elementsize));
    clean_dcache((unsigned int)buffer, queuesize * elementsize);
    
    //flush_dcache_range((unsigned long)result->first, ALIGN_END_ADDR(struct QH, result->first, queuesize));
    clean_dcache((unsigned int)result->first, sizeof(struct QH) * queuesize);
    
    //flush_dcache_range((unsigned long)result->tds, ALIGN_END_ADDR(struct qTD, result->tds, queuesize));
    clean_dcache((unsigned int)result->tds, sizeof(struct qTD) * queuesize);
    ...
    list->qh_link = (unsigned long)result->first | QH_LINK_TYPE_QH;

    //flush_dcache_range((unsigned long)result->last, ALIGN_END_ADDR(struct QH, result->last, 1));
    clean_dcache((unsigned int)result->last, sizeof(struct QH));
    
    //flush_dcache_range((unsigned long)list, ALIGN_END_ADDR(struct QH, list, 1));
    clean_dcache((unsigned int)list, sizeof(struct QH));
...
fail3:
    aligned_free(result->tds);

fail2:
    aligned_free(result->first);
...
static void *_ehci_poll_int_queue(struct usb_device *dev, struct int_queue *queue)
{
    ...
    clean_dcache_nowrite((unsigned int)cur_td, sizeof(struct qTD));

    token = cur_td->qt_token;
    if (QT_TOKEN_GET_STATUS(token) & QT_TOKEN_STATUS_ACTIVE)
    {
    ...
    //invalidate_dcache_range((unsigned long)cur->buffer, ALIGN_END_ADDR(char, cur->buffer, queue->elementsize));
    clean_dcache_nowrite((unsigned int)cur->buffer, queue->elementsize);
...
static int _ehci_destroy_int_queue(struct usb_device *dev, struct int_queue *queue)
{
    struct ehci_ctrl *ctrl = ehci_get_ctrl(dev);
    int result = -1;
    ...
    while (!(cur->qh_link & QH_LINK_TERMINATE))
    {
        ...
        if (NEXT_QH(cur) == queue->first)
        {
            ...
            //flush_dcache_range((unsigned long)cur, ALIGN_END_ADDR(struct QH, cur, 1));
            clean_dcache((unsigned int)cur, sizeof(struct QH));
            result = 0;
            break;
        }
...
out:
    aligned_free(queue->tds);
    aligned_free(queue->first);
...
int submit_int_msg(struct usb_device *dev,
                   ...
                   bool nonblock)
{
    return _ehci_submit_int_msg(dev, pipe, buffer, length, interval, nonblock);
}
...
struct int_queue *create_int_queue(struct usb_device *dev,
                                   ...
                                   int interval)
{
    return _ehci_create_int_queue(dev, pipe, queuesize, elementsize, buffer, interval);
}
...
#if (DM_USB)
static int ehci_submit_control_msg(struct udevice *dev,
...
static int ehci_submit_bulk_msg(struct udevice *dev,
                                struct usb_device *udev,
                                unsigned long pipe,
                                void *buffer,
                                int length)
...
static int ehci_submit_int_msg(struct udevice *dev,
                               struct usb_device *udev,
                               unsigned long pipe,
                               void *buffer,
                               int length,
...
{
    DEBUG("%s: dev='%s', udev=%p\n", __func__, dev->name, udev);
    return _ehci_submit_int_msg(udev, pipe, buffer, length, interval, nonblock);
}
...
{
    DEBUG("%s: dev='%s', udev=%p\n", __func__, dev->name, udev);
    return _ehci_create_int_queue(udev, pipe, queuesize, elementsize, buffer, interval);
}

static void *ehci_poll_int_queue(struct udevice *dev,
                                 struct usb_device *udev,
                                 struct int_queue *queue)
{
    DEBUG("%s: dev='%s', udev=%p\n", __func__, dev->name, udev);
...
static int ehci_destroy_int_queue(struct udevice *dev,
                                  struct usb_device *udev,
                                  struct int_queue *queue)
{
    DEBUG("%s: dev='%s', udev=%p\n", __func__, dev->name, udev);
...
int ehci_register(struct udevice *dev,
