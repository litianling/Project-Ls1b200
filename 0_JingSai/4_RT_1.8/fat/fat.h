...
typedef struct
{
	uint8_t	*fatbuf;			/* 当前 FAT 缓存区 */
	int	     fatsize;			/* FAT 比特位大小 */
	uint32_t fatlength;			/* 扇区中 FAT 的长度 */
	uint16_t fat_sect;			/* FAT的起始扇区 */
	uint8_t	 fat_dirty;      	/* 如果 fatbuf 被修改，则设置 */
	uint32_t rootdir_sect;		/* 根目录的起始扇区 */
	uint16_t sect_size;			/* 扇区大小（以字节为单位） */
	uint16_t clust_size;		/* 扇区中集群的大小 */
	int	     data_begin;		/* 第一个簇的扇区，可以是负数 */
	int	     fatbufnum;			/* 由 get_fatent 使用，初始化为 -1 */
	int	     rootdir_size;		/* 非FAT32根目录大小 */
	uint32_t root_cluster;		/* FAT32 的第一组根目录 */
	unsigned total_sect;		/* 扇区的数量 */
	int	     fats;				/* FATs数量 */
} fsdata_t;
