...
void Mouse_logical_coordinates_reset(void)
{
	...
	fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,15);    // 显示白色方框（光标）
    return;
}
...
static int usb_mouse_irq_worker(struct usb_device *dev)
{
    ...
    */
    ...
    {
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,0);     // 刷新黑色方框（去掉光标）
        if(data->new_[1] >= 127)
        {
            if(Col > 0)
            {
        ...
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,15);    // 刷新白色方框（新的光标）
    }
