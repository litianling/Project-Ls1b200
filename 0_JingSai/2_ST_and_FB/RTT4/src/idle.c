...
{
    /* 循环直到没有死线程。 因此，一次调用 rt_thread_idle_excute 将完成所有清理工作。 */
...
{
    rt_ubase_t i;
    char tidle_name[RT_NAME_MAX];               // 存储线程名字的字符串数组

    for (i = 0; i < _CPUS_NR; i++)              // 多线程CPU就创建多个空线程，此处为单线程CPU。
    {
        rt_sprintf(tidle_name, "tidle%d", i);   // 创建线程名字tidle+i
        rt_thread_init(&idle[i],
                tidle_name,
                rt_thread_idle_entry,           // 空线程入口
                RT_NULL,
                &rt_thread_stack[i][0],
