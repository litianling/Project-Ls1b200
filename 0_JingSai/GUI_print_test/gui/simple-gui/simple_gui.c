...
TButton *btn_buffer = NULL;
TGrid *grid_buffer = NULL;
...
void draw_button(TButton *p_btn,int mydef)
{
    TRect   *rt;
    int      w, h, j, cy1, cy2;
    unsigned color;
	...
	if (((!p_btn->layout_changed)||(p_btn == NULL)) && (mydef == 0))
    	return;
    ...
    w = rt->right  - rt->left;			/* 宽 */
    h = rt->bottom - rt->top;			/* 高 */
...
void draw_button_mydef(TButton *p_btn)
{
    TRect   *rt;
    int      w, h, j, cy1, cy2;
    unsigned color;
...
TGrid *create_grid(TRect *rect, int rows, int cols, int guid, int group)
{
	...
	grid_calcurate_cell_rect_internal(p_grid);     // 计算矩形内部的网格
...
static void paint_grid(TGrid *grid,int mydef)
{
	TRect   *grid_r;
	TColumn *p_column;
	int      i;

	if (((!grid->layout_changed) || (grid == NULL)) && (mydef == 0))
		return;
 ...
 */
void init_simple_gui_queue(void)
{
	font_desc_t *font_desc = get_font_desc((const unsigned char *)"A", 0);     // 表格字体设置为大写字母
	if (font_desc != NULL)
	{
		m_asc_font_width  = font_desc->width;                                  // 重新设定字体大小（像素：长*宽）
		m_asc_font_height = font_desc->height;
	}

	TAILQ_INIT(&gui_buttons);                                                  // 设置队列的起始指针
...
void paint_active_buttons_mydefine(void)
{
    TButton *btn_mydef = btn_buffer;
    TAILQ_FOREACH_FROM(btn_mydef, &gui_buttons, next)       // 循环初始条件优先使用已有的值，（从缓存中加载出来的）
    {
        draw_button(btn_mydef,1);
    }
}

void paint_active_grids_mydefine(void)
{
    TButton *grid_mydef = grid_buffer;
    TAILQ_FOREACH_FROM(grid_mydef, &gui_grids, next)       // 循环初始条件优先使用已有的值，（从缓存中加载出来的）
    {
        paint_grid(grid_mydef,1);
        break;
    }
}
