                             ...
                             int pressed)
{
    ...
    {
        keycode = 1;
        //debug("only left click\n");
        fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,1);    // 显示蓝色块
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,15);   // 刷新白色方框（光标）
    ...
    {
        keycode = 2;
        //debug("only right click\n");
        fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,0);     // 显示黑色块
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,15);    // 刷新白色方框（光标）
        usb_mouse_put_queue(data, keycode);
    }
...
static int usb_mouse_irq_worker(struct usb_device *dev)
{
    ...
    */
    ...
    {
        extern int get_fg_color(void);
        extern int get_bg_color(void);
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_bg_color());     // 刷新黑色方框（去掉光标）背景
        if(data->new_[1] >= 127)
        {
            if(Col > 0)
            {
        ...
        else
        {
            ...
            if(data->new_[0]==1)
                //debug("left click AND move_right\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_fg_color());    // 显示蓝色块 前景
            else if(data->new_[0]==2)
                //debug("right click AND move_right\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_bg_color());     // 显示黑色块 背景
            //else
            //    debug("only move_right\n");
        }
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_fg_color());    // 刷新白色方框（新的光标） 前景
    }
    ...
    if(data->new_[2] != 0)        //Y坐标下的移动
    {
        ...
        if(data->new_[2] >= 127)
        {
            ...
            if(data->new_[0]==1)
                //debug("left click AND move_up\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_fg_color());    // 显示蓝色块 前景
            else if(data->new_[0]==2)
                //debug("right click AND move_up\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_bg_color());     // 显示黑色块 背景
            //else
                //debug("only move_up\n");
        }
        else
        {
            ...
            if(data->new_[0]==1)
                //debug("left click AND move_down\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_fg_color());    // 显示蓝色块 前景
            else if(data->new_[0]==2)
                //debug("right click AND move_down\n");
                fb_fillrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_bg_color());     // 显示黑色块 背景
            //else
                //debug("only move_down\n");
        }
        fb_drawrect(mouse_x,mouse_y,mouse_x+CONS_FONT_WIDTH,mouse_y+CONS_FONT_HEIGHT,get_fg_color());    // 刷新白色方框（新的光标）
    }
