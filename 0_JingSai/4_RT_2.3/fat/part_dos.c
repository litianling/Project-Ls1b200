				                     ...
				                     int part_num, unsigned int disksig)
{
	...
	for (i = 0; i < 4; i++, pt++)
	{
		...
		{
		    printf("These are main part's information.\n\r");
			//print_one_part(pt, ext_part_sector, part_num, disksig);                // 输出提示信息【*】编号、起始扇区数、扇区总数、唯一识别码、类型
		}
	...
	for (i = 0; i < 4; i++, pt++)
	{
		if (is_extended (pt->sys_ind))                                           // 是不是相关区域【*】
		{
            ...
            printf("These are else part's information.\n\r");
		              ...
		              struct disk_partition *info)
{
    ...
    if (ret == 0)
        part_print_dos(dev_desc);                                           // 输出dos部分的信息【*】编号、起始扇区数、扇区总数、唯一识别码、类型
	return ret;
}
...
void dos_read(struct blk_desc *desc,char *fn)
{
    struct disk_partition mypart;
    
    //printf("mbr size=%i\r\n", sizeof(legacy_mbr_t));                    // 打印存储器结构信息所占大小
    //printf("partition size=%i\r\n", sizeof(struct partition));

    if (part_get_info_dos(desc, 1, &mypart) == 0)                       // 获取并打印dos部分的信息【*】编号、起始扇区数、扇区总数、唯一识别码、类型 <隐藏>
    {
        if (fat_set_blk_dev(desc, &mypart) == 0)                        // 设置块存储器设备【*】检查：文件配置表是否有效、是不是一个DOS卷、磁盘的文件系统格式
        {
            if (fat_exists(fn))                                         // 判断是否存在文件【*】
            {
                ...
                char buf[2048];
                printf("found file: %s\r\n", fn);
                count = file_fat_read(fn, buf, 2048);                   // 将文件中的信息读入缓存区buf【*】
                int offset=0;
                while(*(buf+offset))                                    // 显示文件信息
                {
                    printf("%s\r\n", buf+offset);
                    offset+=126;
                }
...
//***************************************ls 命令测试**********************************
void dos_read_dir(struct blk_desc *desc,char *fn)
{
    struct disk_partition mypart;
    ...
    if (part_get_info_dos(desc, 1, &mypart) == 0)                      
    {
        if (fat_set_blk_dev(desc, &mypart) == 0)                       
        {
            if (fat_exists_dir(fn))                                     // 改这里【*】
            {
                return;
            }
            else
