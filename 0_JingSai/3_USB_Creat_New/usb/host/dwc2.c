...
#define CONFIG_USB_DWC2_REG_ADDR  0
#define aligned_buffer_addr       0
#define status_buffer_addr        0

#define readl(x)       READ_REG32(x)       // readl(x)
#define writel(a, v)   WRITE_REG32(a, v)   // writel(b, a)

#define clrsetbits_le32(addr, clear, set)  ((*(addr) & ~(clear)) | (set))

#define clrbits_le32(addr, clear)          (*(addr) & ~(clear))
#define setbits_le32(addr, set)            (*(addr) | (set))

#define debug(format, arg...) do {} while (0)
#define dev_info(format, arg...) do {} while (0)
#define dev_warn(format, arg...) do {} while (0)
#define dev_err(format, arg...) do {} while (0)
...
{
#if (DM_USB)
    unsigned char aligned_buffer[DWC2_DATA_BUF_SIZE] __aligned(ARCH_DMA_MINALIGN);
    unsigned char status_buffer[DWC2_STATUS_BUF_SIZE] __aligned(ARCH_DMA_MINALIGN);
...
#endif
    unsigned char in_data_toggle[MAX_DEVICE][MAX_ENDPOINT];
    unsigned char out_data_toggle[MAX_DEVICE][MAX_ENDPOINT];
...
// TODO DEFINE_ALIGN_BUFFER(unsigned char, aligned_buffer_addr, DWC2_DATA_BUF_SIZE, ARCH_DMA_MINALIGN);
// TODO DEFINE_ALIGN_BUFFER(unsigned char, status_buffer_addr, DWC2_STATUS_BUF_SIZE, ARCH_DMA_MINALIGN);
...
static void init_fslspclksel(struct dwc2_core_regs *regs)
{
    ...
    unsigned int hwcfg2 = readl(&regs->ghwcfg2);
    unsigned int hval = (ghwcfg2 & DWC2_HWCFG2_HS_PHY_TYPE_MASK) >> DWC2_HWCFG2_HS_PHY_TYPE_OFFSET;
    unsigned int fval = (ghwcfg2 & DWC2_HWCFG2_FS_PHY_TYPE_MASK) >> DWC2_HWCFG2_FS_PHY_TYPE_OFFSET;
...
static void dwc_otg_flush_tx_fifo(struct dwc2_core_regs *regs, const int num)
{
    ...
    writel(DWC2_GRSTCTL_TXFFLSH | (num << DWC2_GRSTCTL_TXFNUM_OFFSET), &regs->grstctl);
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_TXFFLSH, false, 1000, false);
    if (ret)
...
static void dwc_otg_flush_rx_fifo(struct dwc2_core_regs *regs)
{
    ...
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_RXFFLSH, false, 1000, false);
    if (ret)
        dev_info(/*dev, */ "%s: Timeout!\n", __func__);
...
static void dwc_otg_core_reset(struct dwc2_core_regs *regs)
{
    ...
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_AHBIDLE, true, 1000, false);
    if (ret)
        dev_info(/*dev, */ "%s: Timeout!\n", __func__);

    /* Core Soft Reset */
    writel(DWC2_GRSTCTL_CSFTRST, &regs->grstctl);
    ret = wait_for_bit_le32(&regs->grstctl, DWC2_GRSTCTL_CSFTRST, false, 1000, false);
    if (ret)
        dev_info(/*dev, */ "%s: Timeout!\n", __func__);
...
#if (DM_USB) && defined(CONFIG_DM_REGULATOR)
 ...
 *
 */
static void dwc_otg_core_host_init(struct udevice *dev, struct dwc2_core_regs *regs)
{
    unsigned int nptxfifosize = 0;
    unsigned int ptxfifosize = 0;
    unsigned int hprt0 = 0;
    ...
    if (readl(&regs->ghwcfg2) & DWC2_HWCFG2_DYNAMIC_FIFO)
    {
        ...
        nptxfifosize |= CONFIG_DWC2_HOST_NPERIO_TX_FIFO_SIZE << DWC2_FIFOSIZE_DEPTH_OFFSET;
        nptxfifosize |= CONFIG_DWC2_HOST_RX_FIFO_SIZE << DWC2_FIFOSIZE_STARTADDR_OFFSET;
        writel(nptxfifosize, &regs->gnptxfsiz);

        /* Periodic Tx FIFO */
        ptxfifosize |= CONFIG_DWC2_HOST_PERIO_TX_FIFO_SIZE << DWC2_FIFOSIZE_DEPTH_OFFSET;
        ptxfifosize |= (CONFIG_DWC2_HOST_RX_FIFO_SIZE + CONFIG_DWC2_HOST_NPERIO_TX_FIFO_SIZE) <<
...
static void dwc_otg_core_init(struct dwc2_priv *priv)
{
    ...
    unsigned int ahbcfg = 0;
    unsigned int usbcfg = 0;
    unsigned char brst_sz = CONFIG_DWC2_DMA_BURST_SIZE;
    ...
    clrsetbits_le32(&regs->gi2cctl, DWC2_GI2CCTL_I2CEN | DWC2_GI2CCTL_I2CDEVADDR_MASK,
 ...
 */
static void dwc_otg_hc_init(struct dwc2_core_regs *regs,
                            unsigned char hc_num,
                            ...
                            uint16_t max_packet)
{
    ...
    unsigned int hcchar = (dev_addr << DWC2_HCCHAR_DEVADDR_OFFSET) |
...
static void dwc_otg_hc_init_split(struct dwc2_hc_regs *hc_regs,
                                  unsigned char hub_devnum,
                                  unsigned char hub_port)
{
    unsigned int hcsplt = 0;
                                           ...
                                           struct devrequest *cmd)
{
    ...
    switch (cmd->requesttype & ~USB_DIR_IN)
    {
        ...
        case USB_RECIP_OTHER | USB_TYPE_CLASS:
            ...
            else if ((hprt0 & DWC2_HPRT0_PRTSPD_MASK) == DWC2_HPRT0_PRTSPD_HIGH)
            ...
            *(unsigned int *)buffer = port_status | (port_change << 16);
            len = 4;
            break;
                                    ...
                                    struct devrequest *cmd)
{
    switch (cmd->request)
    {
        case USB_REQ_GET_STATUS:
            return dwc_otg_submit_rh_msg_in_status(priv->regs, dev, buffer, txlen, cmd);

        case USB_REQ_GET_DESCRIPTOR:
            return dwc_otg_submit_rh_msg_in_descriptor(dev, buffer, txlen, cmd);

        case USB_REQ_GET_CONFIGURATION:
            return dwc_otg_submit_rh_msg_in_configuration(dev, buffer, txlen, cmd);
...
static int dwc_otg_submit_rh_msg(struct dwc2_priv *priv,
                                 struct usb_device *dev,
                                 unsigned long pipe,
                                 void *buffer,
                                 int txlen,
...
int wait_for_chhltd(struct dwc2_hc_regs *hc_regs,
                    unsigned int *sub,
                    unsigned char *toggle)
{
    ...
    ret = wait_for_bit_le32(&hc_regs->hcint, DWC2_HCINT_CHHLTD, true, 2000, false);
    if (ret)
        return ret;
...
static int transfer_chunk(struct dwc2_hc_regs *hc_regs,
                          void *aligned_buffer,
                          unsigned char *pid,
                          int in,
                          void *buffer,
...
int chunk_msg(struct dwc2_priv *priv,
              struct usb_device *dev,
              unsigned long pipe,
              unsigned char *pid,
              int in,
              ...
              int len)
{
    ...
    /* Initialize channel */
    dwc_otg_hc_init(regs, DWC2_HC_CHANNEL, dev, devnum, ep, in, eptype, max);
    ...
    if (dev->speed != USB_SPEED_HIGH)
    {
        ...
        unsigned char hub_port;
        unsigned int hprt0 = readl(&regs->hprt0);
        if ((hprt0 & DWC2_HPRT0_PRTSPD_MASK) == DWC2_HPRT0_PRTSPD_HIGH)
        {
            usb_find_usb2_hub_address_port(dev, &hub_addr, &hub_port);
    ...
    do
    {
        ...
        if (complete_split)
        {
            ...
            {
                ret = 0;
                int frame_num = DWC2_HFNUM_MAX_FRNUM & readl(&host_regs->hfnum);
        ...
        else if (do_split)
        {
            ...
            {
                ssplit_frame_num = DWC2_HFNUM_MAX_FRNUM & readl(&host_regs->hfnum);
                ret = 0;
                complete_split = 1;
            }
        }
...
int _submit_bulk_msg(struct dwc2_priv *priv,
                     struct usb_device *dev,
                     unsigned long pipe,
                     void *buffer,
                     int len)
...
static int _submit_control_msg(struct dwc2_priv *priv,
                               struct usb_device *dev,
                               unsigned long pipe,
                               void *buffer,
                               int len,
                               struct devrequest *setup)
{
    ...
    if (devnum == priv->root_hub_devnum)
    {
        ...
        return dwc_otg_submit_rh_msg(priv, dev, pipe, buffer, len, setup);
    }
    ...
    if (buffer)
    {
        ...
        do
        {
            ret = chunk_msg(priv, dev, pipe, &pid, usb_pipein(pipe), buffer, len);
            act_len += dev->act_len;
            buffer += dev->act_len;
            len -= dev->act_len;
        } while (ret == -EAGAIN);
    ...
    do
    {
        ret = chunk_msg(priv, dev, pipe, &pid, status_direction, priv->status_buffer, 0);
    } while (ret == -EAGAIN);
...
int _submit_int_msg(struct dwc2_priv *priv,
                    struct usb_device *dev,
                    unsigned long pipe,
                    void *buffer,
                    int len,
...
int submit_int_msg(struct usb_device *dev,
                   ...
                   bool nonblock)
{
    return _submit_int_msg(&local, dev, pipe, buffer, len, interval, nonblock);
}
...
#if (DM_USB)
static int dwc2_submit_control_msg(struct udevice *dev,
...
static int dwc2_submit_bulk_msg(struct udevice *dev,
                                struct usb_device *udev,
                                unsigned long pipe,
                                void *buffer,
                                int length)
...
static int dwc2_submit_int_msg(struct udevice *dev,
                               struct usb_device *udev,
