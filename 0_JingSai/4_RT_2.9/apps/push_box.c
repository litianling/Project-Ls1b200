...
#define THREAD_STACK_SIZE       1024*4  //2M  0x00100000
...
void supplement_pushbox(void);  // 目标点补充函数
void success_pushbox(void);     // 通关之后运行
void output_pushbox(void);      // 推箱子显示输出函数
void exit_pushbox(void);        // 推箱子退出函数
...
#if (COLORFUL)
		    if (a[i][j]=='O')
                fb_set_fgcolor_mydef(14);   //浅黄色箱子
            else if(a[i][j]=='M')
			    fb_set_fgcolor_mydef(12);   //浅红色玩家
            else if(a[i][j]=='*')
			    fb_set_fgcolor_mydef(10);   //浅绿色目标点
            else if(a[i][j]=='@')
			    fb_set_fgcolor_mydef(10);   //浅绿色归位点
            else if(a[i][j]=='#')
			    fb_set_fgcolor_mydef(11);   //浅青色墙壁
            else
			    fb_set_fgcolor_mydef(15);   //浅白色其他
#endif
            printf("%c",a[i][j]);
#if (COLORFUL)
            fb_set_fgcolor_mydef(15);     //恢复浅白色
...
void exit_pushbox()     // 推箱子退出函数
{
    extern int show_background;
    show_background = 1;
    usb_mouse_coursor_unlock();
