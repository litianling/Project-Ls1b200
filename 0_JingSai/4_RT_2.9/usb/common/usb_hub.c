...
static int usb_scan_port(struct usb_device_scan *usb_scan)
{
    ...
    portstatus = portsts->wPortStatus;                                      // 端口状态与连接变化
    portchange = portsts->wPortChange;
    ...
    usb_hub_port_connect_change(dev, i);                                    // USB端口连接【*】*********************************************重点
    if (portchange & USB_PORT_STAT_C_ENABLE)
    {
        ...
        if (!(portstatus & USB_PORT_STAT_ENABLE) &&
            ...
            usb_device_has_child_on_port(dev, i))                           //有没有子设备【*】
        {
    ...
    if (portchange & USB_PORT_STAT_C_OVERCURRENT)                           // 电流过载状态
    {
        ...
        /* 只给这一个端口上电 */
        usb_set_port_feature(dev, i + 1, USB_PORT_FEAT_POWER);              // 设置端口特性【*】
...
static int usb_device_list_scan(void)
{
    ...
    {
        /* 一旦列表再次为空就标志着我们完成扫描 */
        if (list_empty(&usb_scan_list))                                 // 判断这个列表是否为空【*】查不到该函数实现1
            goto out;
        ...
        list_for_each_entry_safe(usb_scan, tmp, &usb_scan_list, list)   // 每个入口保险箱的列表【*】查不到该函数实现
        {
            ...
            /* 扫描这个端口 */
            ret = usb_scan_port(usb_scan);                              // 扫描USB端口【*】1
            if (ret)
                goto out;
        }
    }
...
static int usb_hub_configure(struct usb_device *dev)
{
    ...
    if (hubCharacteristics & HUB_CHAR_COMPOUND)                     // 判断其是复合设备的一部分还是独立集线器
    ...
    for (i = 0; i < dev->maxchild; i++)
    {
        ...
        list_add_tail(&usb_scan->list, &usb_scan_list);                             // 列表添加尾部【*】查不到该函数实现1
    }

    /* 现在调用遍历生成列表的扫描代码 */
    ret = usb_device_list_scan();                                                   // 扫描USB设备列表【*】1
    return ret;
}
