...
#define WATCHDOG_RESET()        // ehci watchdog ?

/*
 * Section 2.2.3 - N_PORTS
 */
#if defined(LS1B) || defined(LS1C)
#define MAX_HC_PORTS        2
#else
...
struct ehci_hccr
{
    volatile unsigned int cr_capbase;
#define HC_LENGTH(p)        (((p) >> 0) & 0x00ff)
#define HC_VERSION(p)       (((p) >> 16) & 0xffff)
    volatile unsigned int cr_hcsparams;
#define HCS_PPC(p)          ((p) & (1 << 4))
#define HCS_INDICATOR(p)    ((p) & (1 << 16)) /* Port indicators */
#define HCS_N_PORTS(p)      (((p) >> 0) & 0xf)
    volatile unsigned int  cr_hccparams;
    volatile unsigned char cr_hcsp_portrt[8];
} __attribute__ ((packed, aligned(4)));

struct ehci_hcor
{
    volatile unsigned int or_usbcmd;            // 0xbfe00010 USB主机控制器的命令寄存器
...
#define CMD_RUN         (1 << 0)                /* start/stop HC */
    ...
    volatile unsigned int or_usbsts;            // 0xbfe00014 主机控制器的状态寄存器
#define STS_ASS         (1 << 15)
#define STS_PSS         (1 << 14)
#define STS_HALT        (1 << 12)

    volatile unsigned int or_usbintr;           // 0xbfe00018 USB主机控制器的中断设置寄存器
...
#define INTR_AAE        (1 << 5)                /* Interrupt on async adavance enable */
    ...
    volatile unsigned int or_frindex;           // 0xbfe0001c USB 主机控制器的帧索引寄存器
    volatile unsigned int or_ctrldssegment;     // 0xbfe00020 存放EHCI控制数据结构的地址
    volatile unsigned int or_periodiclistbase;  // 0xbfe00024 存放周期数据帧表的起始地址
    volatile unsigned int or_asynclistaddr;     // 0xbfe00028 存放下一个要被执行的异步队列的起始地址

#if 0
    volatile unsigned int _reserved_0_;
    volatile unsigned int or_burstsize;
    volatile unsigned int or_txfilltuning;
#define TXFIFO_THRESH_MASK      (0x3f << 16)
#define TXFIFO_THRESH(p)        ((p & 0x3f) << 16)
    volatile unsigned int _reserved_1_[6];
#else
    volatile unsigned int _reserved_[9];
#endif

    volatile unsigned int or_configflag;        // 0xbfe00050 配置模式寄存器
#define FLAG_CF         (1 << 0)                /* true:  we'll support "high speed" */

    volatile unsigned int or_portsc[MAX_HC_PORTS];  // 0xbfe00054 端口1状态和控制寄存器
                                                    // 0xbfe00058 端口2状态和控制寄存器
                                                    // Loongson1C 有6个端口?
#define PORTSC_PSPD(x)  (((x) >> 26) & 0x3)
#define PORTSC_PSPD_FS  0x0
#define PORTSC_PSPD_LS  0x1
#define PORTSC_PSPD_HS  0x2
#define PORTSC_FSL_PFSC (1 << 24)   // BIT(24)  /* PFSC bit to disable HS chirping */

    volatile unsigned int or_systune;           /* Used It? */
} __attribute__ ((packed, aligned(4)));
...
#define ehci_readl(x)       READ_REG32(x)       // readl(x)
#define ehci_writel(a, v)   WRITE_REG32(a, v)   // writel(b, a)
...
struct qTD
{
    ...
    unsigned int qt_next;                       /* see EHCI 3.5.1 */
#define QT_NEXT_TERMINATE            1

    unsigned int qt_altnext;                    /* see EHCI 3.5.2 */
    unsigned int qt_token;                      /* see EHCI 3.5.3 */
...
#define QT_BUFFER_CNT                5
    ...
    unsigned int qt_buffer[QT_BUFFER_CNT];      /* see EHCI 3.5.4 */
    unsigned int qt_buffer_hi[QT_BUFFER_CNT];   /* Appendix B */
...
struct ehci_ops
{
    ...
    int (*get_port_speed)(struct ehci_ctrl *ctrl, unsigned int reg);
    void (*powerup_fixup)(struct ehci_ctrl *ctrl, unsigned int *status_reg,
                          unsigned int *reg);
    unsigned int *(*get_portsc_register)(struct ehci_ctrl *ctrl, int port);
...
struct ehci_ctrl
{
    ...
    struct QH qh_list __attribute__((aligned(USB_DMA_MINALIGN)));        // __aligned(USB_DMA_MINALIGN);
    struct QH periodic_queue __attribute__((aligned(USB_DMA_MINALIGN))); // __aligned(USB_DMA_MINALIGN);
    unsigned int *periodic_list;
