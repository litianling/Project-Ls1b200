...
int usb_mouse_get_color(void)
{
    return mouse_color;
}

void usb_mouse_set_color(int color)
{
    mouse_color = color;
}

void usb_mouse_draw_unlock(void)
{
    mouse_draw_lock = 0;
}

void usb_mouse_draw_lock(void)
{
    mouse_draw_lock = 1;
}
...
static int usb_mouse_irq_worker(struct usb_device *dev)
{
    ...
    */
    ...
    {
        restore_mouse_graph(mouse_x,mouse_y);   // 去掉过期光标
        ...
        save_old_mouse_graph(mouse_x,mouse_y);              // 记录原始显示状态
        show_mouse_graph(mouse_x,mouse_y,mouse_color);      // 显示光标
    }
