...
static int disk_read(uint32_t block, uint32_t nr_blocks, void *buf)
{
	...
	ret = blk_dread(cur_dev, cur_part_info.start + block, nr_blocks, buf);     // 读块存储器【*】
...
int fat_set_blk_dev(struct blk_desc *dev_desc, struct disk_partition *info)
{
	...
	/* 确保它具有有效的 FAT 标头 */                // FAT==文件配置表
	if (disk_read(0, 1, buffer) != 1)              // 读取磁盘【*】
	{
		cur_dev = NULL;
		return -1;
	}

	/* 检查它是否实际上是一个 DOS 卷 */            // DOS==磁盘操作系统
...
static void get_name(dir_entry_t *dirent, char *s_name)
{
	...
	if (dirent->lcase & CASE_LOWER_BASE)
		downcase(s_name, (unsigned)(ptr - s_name));       // 小写【*】
...
static uint32_t get_fatent(fsdata_t *mydata, uint32_t entry)
{
	...
	if (CHECK_CLUST(entry, mydata->fatsize))                           // 检验【*】
	{
	...
	if (bufnum != mydata->fatbufnum)
	{
		...
		/* 如果 fatlength 不是 FATBUFBLOCKS 的倍数，则为上限长度 */
		if (startblock + getsize > fatlength)
			getsize = fatlength - startblock;

		startblock += mydata->fat_sect;	/*从磁盘开始的偏移量 */

		/* 将fatbuf写回磁盘 */
		if (flush_dirty_fat_buffer(mydata) < 0)                           // 将fat缓冲区写入块设备【*】
			return -1;

		if (disk_read(startblock, getsize, bufptr) < 0)                   // 磁盘读取【*】
		{
...
/*
 * 将指定集群中的“大小”字节读入“缓冲区”。 成功返回 0，否则返回 -1。
 */
static int
get_cluster(fsdata_t *mydata, uint32_t clustnum, uint8_t *buffer, unsigned long size)
{
	...
	{
		ALLOC_CACHE_ALIGN_BUFFER(uint8_t, tmpbuf, mydata->sect_size);         // 分配空间【*】

		debug("FAT: Misaligned buffer address (%p)\n", buffer);

		while (size >= mydata->sect_size)
		{
			ret = disk_read(startsect++, 1, tmpbuf);                          // 磁盘读取【*】
			if (ret != 1)
			{
			...
			memcpy(buffer, tmpbuf, mydata->sect_size);                       // 数据拷贝【*】
	...
	else
	{
		...
		ret = disk_read(startsect, idx, buffer);                              // 磁盘读取【*】
		if (ret != idx)
		{
			debug("Error reading data (got %d)\n", ret);
			return -1;
	...
	if (size)
	{
		ALLOC_CACHE_ALIGN_BUFFER(uint8_t, tmpbuf, mydata->sect_size);         // 空间分配【*】
		...
		memcpy(buffer, tmpbuf, size);                                         // 数据拷贝【*】
	}
			            ...
			            uint8_t *buffer, loff_t maxsize, loff_t *gotsize)
{
	...
	uint32_t curclust = START(dentptr);                                        // 开始文件夹--文件路径【*】
	...
	*gotsize = 0;
	...
	while (actsize <= pos)
	{
		curclust = get_fatent(mydata, curclust);                              // 获取 FAT (12/16/32) 表中索引“条目”处的条目【*】
		if (CHECK_CLUST(curclust, mydata->fatsize))                           // 检查【*】
		{
			debug("curclust: 0x%x\n", curclust);
	...
	if (pos)
	{
		...
		actsize = min(filesize, (loff_t)bytesperclust);                       // 最小【*】
		tmp_buffer = malloc_cache_aligned(actsize);                           // 分配【*】
		if (!tmp_buffer)
		{
		...
		if (get_cluster(mydata, curclust, tmp_buffer, actsize) != 0)          // 将指定集群中的“大小”字节读入“缓冲区”【*】
		{
			printf("Error reading cluster\n");
			aligned_free(tmp_buffer);                                         // 释放并对其【*】
			return -1;
		}

		filesize -= actsize;
		actsize -= pos;
		memcpy(buffer, tmp_buffer + pos, actsize);                            // 数据拷贝【*】
		aligned_free(tmp_buffer);                                             // 同上【*】
		*gotsize += actsize;
		if (!filesize)
			return 0;
		buffer += actsize;

		curclust = get_fatent(mydata, curclust);                              // 同上【*】
		if (CHECK_CLUST(curclust, mydata->fatsize))                           // 检查【*】
		{
			debug("curclust: 0x%x\n", curclust);
			printf("Invalid FAT entry\n");
	...
	do
	{
		...
		{
			newclust = get_fatent(mydata, endclust);                         // 同上【*】
			if ((newclust - 1) != endclust)
				goto getit;
			if (CHECK_CLUST(newclust, mydata->fatsize))                      // 同上【*】
			{
				debug("curclust: 0x%x\n", newclust);
				printf("Invalid FAT entry\n");
				return -1;
		...
		/* 获取剩余字节 */
		actsize = filesize;
		if (get_cluster(mydata, curclust, buffer, (int)actsize) != 0)        // 同上【*】
		{
			printf("Error reading cluster\n");
			return -1;
		}
...
getit:
		...
		*gotsize += (int)actsize;
		...
		curclust = get_fatent(mydata, endclust);                             // 同上【*】
		if (CHECK_CLUST(curclust, mydata->fatsize))                          // 同上【*】
		{
			debug("curclust: 0x%x\n", curclust);
			printf("Invalid FAT entry\n");
			return -1;
...
/*
 * 将'slotptr'中的文件名信息提取到'l_name'中，
  * 从 l_name[*idx] 开始。
  * 如果找到终止符（零字节）则返回 1，否则返回 0.
 */
...
typedef struct
{
	fsdata_t   *fsdata;        /* 文件系统参数 */
	unsigned    start_clust;   /* 第一个集群 */
	unsigned    clust;         /* 当前集群 */
	unsigned    next_clust;    /* 下一个集群 if remaining == 0 */
	int         last_cluster;  /* 一旦我们读取了最后一个集群就置一 */
	int         is_root;       /* 是不是根目录的迭代器 */
	int         remaining;     /* 当前集群中剩余的凹痕 */

	/* 当前迭代器位置值: */
	dir_entry_t *dent;         /* 当前目录项 */
	char        l_name[VFAT_MAXLEN_BYTES];    /* 长 (vfat) 名称 */
...
static int fat_itr_root(fat_itr_t *itr, fsdata_t *fsdata)
{
	...
	itr->fsdata = fsdata;                      // 文件系统数据
	itr->start_clust = 0;                      // 第一个集群
	itr->clust = fsdata->root_cluster;         // 当前集群是第一组根目录
	itr->next_clust = fsdata->root_cluster;    // 下个集群还是第一组根目录
	itr->dent = NULL;                          // 当前目录项无
	itr->remaining = 0;                        // 当前集群中无剩余的凹痕
	itr->last_cluster = 0;                     // 没有读取完最后一个集群
	itr->is_root = 1;                          // 是根目录迭代器
...
static dir_entry_t *extract_vfat_name(fat_itr_t *itr)
{
	...
	while (seqn--)
	{
		...
		slot2str((dir_slot_t *)dent, buf, &idx);                              // 字符转换【*】
		...
		dent = next_dent(itr);                                                // 下一文件夹【*】
		if (!dent)
			return NULL;
	}

	/*
	 * 我们现在处于短文件名条目。 如果它被标记为已删除，请跳过它。
	 */
	...
	chksum = mkcksum(dent->name, dent->ext);                                   // 计算短名称校验【*】

	/* 校验和不匹配可能意味着删除文件等。跳过它: */
	...
	while (1)
	{
		...
		if (dent->attr & ATTR_VOLUME)
		{
			if ((dent->attr & ATTR_VFAT) == ATTR_VFAT &&
				...
				/* 长文件名 */
				dent = extract_vfat_name(itr);              // 提取vfat名称【*】
				/*
...
/**
 * fat_itr_resolve() - 遍历目录结构来解析请求的路径。
 *
...
static int fat_itr_resolve(fat_itr_t *itr, const char *path, unsigned type)
{
	...
	/* 移除任何额外的前导斜杠: */
	while (path[0] && ISDIRDELIM(path[0]))    // 移除文件路径中额外的前导斜杠（path[0]非空并且不是斜杠）
		path++;
	/* 遍历到末尾了吗? */
	if (strlen(path) == 0)                    // 计算字符串长度【*】移除斜杠后的名字是空的话，说明用户输入的是文件夹，并且已经找到目标文件夹
	{
		if (!(type & TYPE_DIR))               // 但是如果禁用文件夹类型会报错跳过
			return -ENOENT;
		return 0;
	}

    /* next是用户输入的下一条 */
	next = path;                               // 计算路径中的下一个条目
	while (next[0] && !ISDIRDELIM(next[0]))    // 移除next的前导斜杠（next[0]非空并且不是斜杠）
		next++;
	...
	if (itr->is_root)
	{
		    ...
		    (((next - path) == 2) && !strncmp(path, "..", 2)))
		{
			...
			if (next[0] == 0)                              // 如果用户输入路径下一条条目为空
			{
				if (type & TYPE_DIR)                       // type是any，都会执行这个if
					return 0;
				else
					return -ENOENT;                        // 最后一项是文件类型，找到目标文件了
			}
			return fat_itr_resolve(itr, next, type);       // 文件夹中还有东西，还没有遍历到终点，可以继续递归遍历
		}
	}

    /* 非根目录路径的处理 */
	while (fat_itr_next(itr))                              // 迭代器跳转到目录中的下一个条目【*】
	{
		int match = 0;
		unsigned n = max(strlen(itr->name), (size_t)(next - path));   // 路径的最大长度【*】

		/* 检查长名称和短名称: */
		if (!strncasecmp(path, itr->name, n))             // 迭代器所在位置并未偏离用户设置的路径【*】
			match = 1;
		else if (itr->name != itr->s_name &&              // 迭代器name和s_name不一致，并且s_name也偏离路径【*】
			 !strncasecmp(path, itr->s_name, n))
			match = 1;
		if (!match)                                       // 偏离路径就修正，跳转到同文件夹中的下一个条目
			continue;

		if (fat_itr_isdir(itr))                           // 当前迭代器是文件夹，继续递归遍历
		{
			fat_itr_child(itr, itr);                      // 创建一个新的迭代器用于递归
			return fat_itr_resolve(itr, next, type);      // 遍历目录结构来解析请求的路径【*】
		}
		else if (next[0])                                 // 迭代器是文件并且下一个条目非空，那么我们的情况如下：/path/to/realfile/nonsense，找不到该文件
		{
			debug("bad trailing path: %s\n", next);
			return -ENOENT;
		}
		else if (!(type & TYPE_FILE))                     // 迭代器是文件并且下一个条目非空，找到了目标文件，但是如果禁用文件类型会报错跳过
		{
			return -ENOTDIR;
		}
		else                                              // 迭代器是文件并且下一个条目为空
		{
			return 0;
		}
	}
...
//***************************************ls 命令测试************************************************
static int fat_itr_resolve_dir(fat_itr_t *itr, const char *path, unsigned type)
{
	const char *next;
	...
	if (strlen(path) == 0)                    // 计算字符串长度【*】移除斜杠后的名字是空的话，说明用户输入的是文件夹，并且已经找到目标文件夹
	{
		if (!(type & TYPE_DIR))               // 但是如果禁用文件夹类型会报错跳过
			return -ENOENT;
#if 1
        while(fat_itr_next(itr))              // 跳转到同文件夹中下一个文件
            printk("%s\r\n",itr->name);
#endif
		return 0;
	}
	...
	if (itr->is_root)
	{
		    ...
		    (((next - path) == 2) && !strncmp(path, "..", 2)))
		{
			...
			return fat_itr_resolve_dir(itr, next, type);   // 用户输入路径还有东西，还没有遍历到终点，可以继续递归遍历
		}
	}

    /* 非根目录路径的处理 */
	while (fat_itr_next(itr))                              // 迭代器进入目录，指向目录中第一个文件【*】
	{
		int match = 0;
		unsigned n = max(strlen(itr->name), (size_t)(next - path));   // 路径的最大长度【*】
		...
		if (fat_itr_isdir(itr))                           // 当前迭代器是文件夹，继续递归遍历
		{
			fat_itr_child(itr, itr);                      // 创建一个新的迭代器用于分支，保存分叉的父节点
			return fat_itr_resolve_dir(itr, next, type);  // 遍历目录结构来解析请求的路径【*】
		}
		else if (next[0])                                 // 迭代器是文件并且下一个条目非空，那么我们的情况如下：/path/to/realfile/nonsense，找不到该文件
		{
			debug("bad trailing path: %s\n", next);
...
{
	fsdata_t fsdata;                                       // file system data文件系统数据（结构体）：缓存区、扇区、根目录信息等
	fat_itr_t *itr;                                        // File Allocation Table iterator 文件配置表（FAT是一种文件系统）迭代器：文件系统数据、集群信息、迭代器位置
	int ret;

	itr = malloc_cache_aligned(sizeof(fat_itr_t));         // 迭代器iterator（itr）空间分配【*】
	if (!itr)                                              // 迭代器空间分配失败，返回0
		return 0;
	ret = fat_itr_root(itr, &fsdata);                      // 将迭代器初始化为从根目录开始【*】
	if (ret)
		goto out;

	ret = fat_itr_resolve(itr, filename, TYPE_ANY);        // 遍历目录结构来解析请求的路径【*】

	aligned_free(fsdata.fatbuf);                           // 释放文件数据缓存区【*】

out:
	aligned_free(itr);                                     // 释放齐itr【*】
	return ret == 0;
}

//***************************************ls 命令测试**********************************
int fat_exists_dir(const char *filename)
{
	fsdata_t fsdata;                                     
	fat_itr_t *itr;                                       
	int ret;
	...
	if (ret)
		goto out;
	ret = fat_itr_resolve_dir(itr, filename, TYPE_ANY);    // 改这里【*】
...
// 要读取文件名字，位置0，缓存区，读取最大字节数，actread
		             ...
		             loff_t maxsize, loff_t *actread)
{
	...
	itr = malloc_cache_aligned(sizeof(fat_itr_t));                             // 存储器空间分配【*】
	if (!itr)
		return -ENOMEM;
	ret = fat_itr_root(itr, &fsdata);                                          // 初始化一个迭代器以从根目录开始【*】
	if (ret)                                                                   // 初始化失败就释放这个迭代器
		goto out_free_itr;

	ret = fat_itr_resolve(itr, filename, TYPE_FILE);                           // 遍历目录结构来解析请求的路径【*】————核心.用itr定位文件位置
	if (ret)
		goto out_free_both;

	debug("reading %s at pos %llu\n", filename, pos);

	/* 用于保存分配给 malloc 池的默认最大集群大小内存 */
	dir_entry_t *dentptr = itr->dent;                                          // 文件夹入口，当前目录项
	ret = get_contents(&fsdata, dentptr, pos, buffer, maxsize, actread);       // 获取内容【*】——————————————重点

out_free_both:
	aligned_free(fsdata.fatbuf);                                               // 释放并对齐空间【*】
out_free_itr:
	aligned_free(itr);                                                         // 同上【*】
	return ret;
}
...
{
	loff_t actread;    // 反馈信息——实际读取的字节数
	int ret;

	ret =  file_fat_read_at(filename, 0, buffer, maxsize, &actread);           // 读文件【*】
	if (ret)
		return ret;
	else
		return actread;
...
static int disk_write(uint32_t block, uint32_t nr_blocks, void *buf)
{
	...
	ret = blk_dwrite(cur_dev, cur_part_info.start + block, nr_blocks, buf);    // 块存储器写入【*】
	if (nr_blocks && ret == 0)
		return -1;
...
static int flush_dirty_fat_buffer(fsdata_t *mydata)
{
	...
	/* 写入 FAT 缓冲区 */
	if (disk_write(startblock, getsize, bufptr) < 0)                   // 磁盘写入【*】
	{
	...
	if (mydata->fats == 2)
	{
		...
		if (disk_write(startblock, getsize, bufptr) < 0)              // 同上【*】
		{
